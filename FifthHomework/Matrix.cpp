...
{	
	this->matrix = new std::vector<std::vector<double>>(n, std::vector<double>(m, 0.0));
	this->noOfRows = n;
	this->noOfColumns = m;
...
{
	this->matrix = new std::vector<std::vector<double>>(n, std::vector<double>(n, 0.0));
	this->noOfRows = n;
	this->noOfColumns = n;
...
Matrix::Matrix(const Matrix & matrixIn) {
	...
	this->noOfRows = matrixIn.getNoOfRows();
	this->noOfColumns = matrixIn.getNoOfColumns();
...
{
	Matrix column = Matrix(1, (int) this->noOfRows);

	for (int i = 0; i < this->noOfRows; i++) {

		column.getMatrix()[0][i] = (*(this->matrix))[i][index];
...
Matrix Matrix::operator!()
{
	Matrix result = Matrix((int) this->noOfRows, (int) this->noOfColumns);
	Matrix P;
	Matrix A = this->decompositionLUP(P);

	for (int i = 0; i < this->noOfRows; i++) {

		Matrix e = Matrix(this->getNoOfRows(), 1);

		e[i][0] = 1.0;

		Matrix y = A.forwardSupsitution(P *e);
		Matrix x = A.backwardSupsitution(y);
